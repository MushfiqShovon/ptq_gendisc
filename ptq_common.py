...
def quantize_model(
    ...
    quant_model = quantize_fn(model.to(device), **quantize_kwargs).to(device)
    return quant_model
...
def create_quant_maps(
        ...
        device='cuda'):  # Ensure device parameter is included
    """
    ...
    weight_quant = weight_quant.let(**weight_bit_width_dict, device=device)  # Include device
    ...
    if act_bit_width is not None:
        ...
        act_quant = act_quant.let(**act_bit_width_dict, device=device)  # Include device
        sym_act_quant = sym_act_quant.let(**act_bit_width_dict, device=device)  # Include device
        per_tensor_act_quant = per_tensor_act_quant.let(**act_bit_width_dict, device=device)  # Include device
    else:
        act_quant = None
        sym_act_quant = None
    ...
    if act_quant is not None:
        act_quant = act_quant.let(
            **{
                'high_percentile_q': act_quant_percentile, 'dtype': dtype, 'device': device})  # Include device
    ...
    quant_wbiol_kwargs = {
        **weight_quant_dict,
        'dtype': dtype,
        'device': device,  # Ensure device is included
        'return_quant_tensor': False,
        'bias_quant': bias_quant}

    # Quantization kwargs for Embedding layers
    quant_embedding_kwargs = {
        **weight_quant_dict,
        'return_quant_tensor': False,
        'device': device  # Include device
    }

    # Quantization kwargs for LSTM layers
    quant_lstm_kwargs = {
        **weight_quant_dict,
        'bias_quant': bias_quant,
        'return_quant_tensor': False,
    ...
    # Include the device parameter in the layer map
    quant_layer_map = {
        torch.nn.Linear: (qnn.QuantLinear, {
            **layerwise_quant_wbiol_kwargs,
            'device': device  # Ensure device is included
        }),
        torch.nn.Embedding: (qnn.QuantEmbedding, {
            **quant_embedding_kwargs,
            'device': device  # Ensure device is included
        }),
        torch.nn.LSTM: (qnn.QuantLSTM, {
            **quant_lstm_kwargs,
            'device': device  # Ensure device is included
        }),
    ...
    quant_layerwise_layer_map = quant_layer_map  # For layerwise quantization
